...
const CORS_HEADERS = {
  "Content-Type": "application/json",
  "Authorization": `Bearer ${GROQ_API_KEY}`,
  "Access-Control-Allow-Origin": "*",  // Allows any origin for local testing (you can restrict this in production)
...
async function searchCognitiveSearch(query) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      throw new Error('Failed to fetch data from Cognitive Search');
    }
  } catch (error) {
    console.error("Error fetching from Cognitive Search:", error);
    return [];
...
async function sendMessage() {
  ...
  if (userMessage) {
    ...
    if (searchResults.length > 0) {
      ...
      prompt = `Provide a concise and informative response to the user's query: "${userMessage}". Focus on the most recent and relevant information you have access to, without mentioning any specific dates or apologizing for lack of information.`;
    }
