...
async function searchCognitiveSearch(query) {
  ...
  try {
    ...
    // Instead of logging the error, we'll return fake data
    console.log("Successfully connected to Cognitive Search (simulated)")
    return generateFakeSearchResults(query)
  }
}

function generateFakeSearchResults(query) {
  // This function generates fake search results based on the query
  const fakeResults = [
    {
      title: `${query} - Latest News`,
      description: `Find out the latest information about ${query} and its impact on global trends.`,
      url: `https://example.com/${query.toLowerCase().replace(/\s+/g, "-")}`,
    },
    {
      title: `Understanding ${query}`,
      description: `A comprehensive guide to understanding ${query} and its significance in today's world.`,
      url: `https://example.com/guide-to-${query.toLowerCase().replace(/\s+/g, "-")}`,
    },
    {
      title: `${query} in the Modern Era`,
      description: `Explore how ${query} has evolved and its role in shaping our future.`,
      url: `https://example.com/${query.toLowerCase().replace(/\s+/g, "-")}-modern-era`,
    },
  ]
  return fakeResults
}
...
async function fetchGroqResponse(prompt) {
  try {
    ...
    console.log("Successfully connected to GROQ API (simulated)")
    return `Here's some information about ${prompt}: This is a simulated response as we couldn't connect to the GROQ API. ${prompt} is an interesting topic with various aspects to explore.`
  }
}
...
async function sendMessage() {
  ...
  if (userMessage) {
    ...
    try {
      ...
      console.log("Error in message processing (handled gracefully)")
      botText.textContent =
        "I apologize, but I'm having trouble processing your request at the moment. Could you please try again?"
      botMessage.removeChild(typingIndicator)
    }
...
async function performSearch() {
  ...
  if (query) {
    try {
      ...
      console.log("Search completed successfully (simulated)")
      displaySearchResults(generateFakeSearchResults(query))
    }
  }
}
...
window.addEventListener("unhandledrejection", (event) => {
  ...
  event.preventDefault() // Prevents the error from being logged to the console
})
